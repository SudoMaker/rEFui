...
>
> While rEFui supports different JSX transforms, the preferred approach for maximum flexibility is the **Classic Transform**. This pattern allows you to swap or wrap renderers on a per-component basis. But for generic usage, automatic transform can cover most scenarios. For a detailed guide on setting up both Classic and Automatic runtimes, see the [JSX Setup documentation](../guides/jsx-setup.mdx). In this guide, we'll default to Automatic runtime for the ease of usage and alignment to most other JSX based frameworks like `React` and `Solid.js`.
...
- [Basic Components](../reference/components/basic.mdx) (`If`, `For`, `Fn`, `Dynamic`, `Render`, `memo`, `useMemo`, `keepAlive`, `useKeepAlive`)
- [Async Components](../reference/components/async.mdx) (`Async`, `Suspense`, `Transition`, `lazy`)
- [Extra Components](../reference/components/extras.mdx) (`UnKeyed`, `Cached`, `Portal`, `Parse`, `defineCustomElement`)
