...
The `<For>` component efficiently renders and updates lists of items. Its child is an item method that receives the `item` and returns a renderable node; the method runs directly in the retained scope for that item rather than as a separate component.
...
- [**Agent Skills**](guides/agent-skills.mdx): Use the bundled skill pack to implement rEFui requirements faster.
