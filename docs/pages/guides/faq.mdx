...
rEFui is fine‑grained: signals track exactly the dependencies they touch, so unaffected subtrees don’t re-render. Prefer storing state in signals close to where it’s read, and rely on built-in tracking instead of manual memoization. Use `For`/`UnKeyed` for lists to avoid diffing, `createCache` for reusable heavy fragments, and `keepAlive` only when a concrete subtree must survive temporary detachment.
...
`memo`/`useMemo` cache one function result; `keepAlive` retains a concrete rendered subtree, and `useKeepAlive` prepares a scope-safe keep-alive factory for use across component instances. Everyday updates normally need none of them.

## Why doesn’t my computed signal update?
Computed/`watch`/`useEffect` track signals when they’re read. A dependency behind an early return is discovered when an already-tracked condition changes and makes that branch reachable:
```js
...
```
If a value must be observed even while its branch is skipped, call `.touch()` before the branch or use `connect` with explicit sources.
...
Use the [DOM renderer](dom-renderer.mdx) for browsers, the HTML renderer for SSR, and Reflow when you want renderer-agnostic logic or to swap renderers later. For new projects, Vite + automatic JSX is the fastest path; switch to classic JSX when you need explicit renderer control.
...
Use `createPortal()` from `refui/extras`; it returns `[Inlet, Outlet]`. Render `<Inlet>` where you produce content and `<Outlet>` where you consume it. Portals are renderer-agnostic. With the Reflow renderer, you can even input from a DOM renderer and output to an HTML renderer, and vice versa.
