...
- [Signals](../concepts/signals.mdx) — deep dive into batching, helpers, and utilities like `nextTick` and `bind`.
- [Components](../concepts/components.mdx) — retained component patterns (`<If>`, `<For>`, `<Async>`), useful while replacing template syntax from other frameworks.
- [DOM Renderer](dom-renderer.mdx) — covers directives, events, and macro registration referenced in this guide.
- [JSX Setup](jsx-setup.mdx) — configuration for classic vs automatic runtimes, required when adjusting build pipelines during migration.
