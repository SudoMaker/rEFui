...
### Computed dependency tracking through branches
Dependencies are discovered when control flow reaches their reads.
```js
const info = computed(() => {
	const t = track.value
	if (!t) return 'Ready'
	return `${t.title} - ${metadata.value}`
})
```
Here `metadata` starts being observed when `track` becomes truthy. Touch it before the branch only when changes to `metadata` must rerun the computation while `track` is still falsy.
