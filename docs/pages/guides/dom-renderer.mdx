...
For instructions on how to set up JSX, please see the [JSX Setup Guide](jsx-setup.mdx). It covers both the preferred **Classic Transform** and the **Automatic Runtime**.

## "Hello, World!" Example (Classic Transform)

Here is a complete example of rendering a component to the DOM using the preferred classic transform. You will first need to [configure your build tool](jsx-setup.mdx#classic-transform-preferred).
...
> - <code>{`<div>{$(() => \`Count is \${count.value}\`)}</div>`}</code> - **Correct**: Wrap computed strings or `.value` access.
...
> **Note**: The `class:` directive is available when using the browser preset (`refui/browser`). For more information about presets and directives, see the [Presets documentation](../reference/presets.mdx#browser).
...
- Check [here](../reference/presets.mdx#browser)
