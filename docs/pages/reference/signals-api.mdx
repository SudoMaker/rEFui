...
### `contextValid`
Boolean flag for the context currently executing. A frozen or captured callback runs with `contextValid === false` after its owner has disposed, then the previous ambient value is restored when that callback returns. Runtime features such as async boundaries use it to avoid rendering into a disposed scope.
