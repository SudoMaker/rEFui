...
Low-level lifecycle and reactive-effect scope. Constructing a scope does not run its effect; call `run()` to execute it and collect signal dependencies. A cleanup function returned by the effect runs before the next execution and when `destroy()` is called.
...
Returns one function that disposes every connection created by the call.
...
Creates a derived signal from an object property. The derived signal's content updates when the original signal changes, or when the corresponding property is itself a signal. A nullish source clears the derived value to `undefined`. Writing the derived signal does not update the original signal's property.
...
If one or more queued effects throw, the scheduler continues draining runnable work before rejecting. One failure is rethrown directly; multiple failures are reported as an `AggregateError`.
