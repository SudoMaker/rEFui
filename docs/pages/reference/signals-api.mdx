...
Signal writes use `!==` change detection. Repeated `NaN` writes therefore trigger updates, while `+0` and `-0` are treated as equal.
...
Computed signals are eager: their computation runs immediately and whenever a dependency changes. Create them inside a component, an `EffectScope` (for example, inside `watch()` or `useEffect()`), or a `collectDisposers()` scope when their lifetime is not application-global. A root-level computed remains connected while its reachable source graph retains it.
...
This is useful when you need to manually force a computed signal to re-evaluate its computation, for example when external dependencies that aren't tracked by the signal system may have changed. Refresh reruns the computed signal's own effect scope, preserving its original input and collecting any newly reached signal dependencies without attaching them to the caller. If that computation was already queued, the synchronous refresh consumes the pending execution rather than running it twice.
...
This is an explicit connection: signal reads inside `effect` do not add dependencies. Use `watch()` or `useEffect()` when the callback should automatically subscribe to every signal it reads.
...
```javascript
const dispose = mySignal.connect(() => console.log('Signal changed'))

// Connect without running immediately
const disposeDeferred = mySignal.connect(() => console.log('Signal changed'), false)
...
- `trueValue` and `falseValue` can be plain values or signals. Functions are returned as values; they are not invoked.
- Signal arguments are both subscribed up front so either branch remains reactive after the condition changes.
...
Combines logical AND and OR in a single helper, equivalent to `(source && andValue) || orValue`.

- A falsy source resolves to `orValue`.
- A truthy source resolves to `andValue` when `andValue` is truthy, otherwise it falls through to `orValue`.
- Signal arguments are both subscribed up front.
...
Variant of `.andOr` that negates the base signal first, equivalent to `(!source && andValue) || orValue`.

- A truthy source resolves to `orValue`.
- A falsy source resolves to `andValue` when `andValue` is truthy, otherwise it falls through to `orValue`.
...
Low-level lifecycle and reactive-effect scope. Constructing a scope does not run its effect; call `run()` to execute it and collect signal dependencies. A cleanup function returned by the effect runs before the next execution and when `destroy()` is called. Cleanup before a rerun retains the effect as its lifecycle owner, while all cleanup runs have dependency collection disabled so teardown-only signal reads do not make an effect reactive to those signals.

Effects and disposal callbacks created while the scope is running belong to it. Destroying the scope releases those children and makes later queued runs inert. Component instances extend `EffectScope`, so component-owned effects use the same lifecycle representation.
...
Signals reached on later runs are tracked as well, so short-circuited expressions such as `a.value && b.value` begin listening to `b` after `a` becomes truthy. If later control flow stops reading a dependency, its existing subscription may cause one final run when that signal changes; because the effect does not read it again, that stale subscription is then retired.

Each run is also a lifecycle scope. Child effects and `onDispose` callbacks created during a run are cleaned before the next run and when the parent watch is disposed. Deliberately long-lived work should be created outside the rerunning watch or placed in a separately managed detached scope.

If an execution throws, the effect is destroyed after its registered cleanup finishes. This prevents a failed rerun from remaining active with a partially collected dependency graph; create a new effect explicitly if application error handling should retry it.
...
Only the supplied `signals` schedule the callback. Reads performed inside `effect` are untracked; use `watch()` or `useEffect()` for automatic dependency discovery.
...
```javascript
const dispose = connect([signal1, signal2], () => {
	console.log('Signals changed')
})

// Connect without running immediately
const disposeDeferred = connect([signal1, signal2], () => {
	console.log('Signals changed')
}, false)
...
Returns one function that disposes every connection created by the call. Registration is transactional: because each signal invokes the callback eagerly, a callback failure during setup disposes every connection that was already created before rethrowing the error.

```javascript
const dispose = listen([signal1, signal2], () => {
...
- Returns: `[onAction, trigger, touch]`
	- `onAction(listener)` registers a callback bound to the current reactive scope. Do **not** call `onAction` from inside reactive effects or loops—each call adds another listener that will fire on every trigger. If you need to ignore future events, gate inside the callback or dispose the surrounding scope.
	  Action listeners are not reactive effects: signals read by a listener do not subscribe that listener, including signals the listener also modifies. Create a `watch()` or `useEffect()` inside the listener when new reactive work is intentional.
	- `trigger(nextValue?)` updates the internal signal and schedules every registered listener. Like other signal updates, multiple triggers within the same tick are batched and delivered once using the latest value.
...
Creates a derived signal from an object property. The derived signal's content updates when the original signal changes, or when the corresponding property is itself a signal. Property propagation is treated as derived work and settles before user effects run. A nullish source clears the derived value to `undefined`. Writing the derived signal does not update the original signal's property.
...
### `extract(signal, ...keys)`
Extracts properties from a signal into separate signals. The extracted signals update only when the original signal changes. Writing the extracted signals does not update the original signal's properties.
...
### `makeReactive(object)`
Creates a new accessor-backed object. Properties whose input values are signals become getters and setters that read and write those signals; non-signal properties are copied as plain values. This helper does not create a JavaScript `Proxy` and does not make nested plain objects reactive.
...
Creates conditional matching based on signal values. When the match condition is itself a signal, its match state settles as derived work before user effects run.
...
If the `effect` function returns another function, that returned function will be used as a `cleanup` handler. The cleanup is called right before the effect re-runs, and also when the component/scope is disposed. Signal reads performed only by cleanup do not become dependencies of the effect.
...
### `collectDisposers(fn, cleanup?, disposers?, ownerDisposers?)`
Collects disposers created while `fn` runs and returns a function that disposes the collected scope. The optional `cleanup` runs before the collected child disposers, and an existing `DisposerStore` can be supplied as the third argument. The optional fourth argument supplies an owner store; pass `null` only for a deliberately detached scope whose lifetime is managed manually. Treat supplied stores as opaque: do not depend on their length, ordering, or entries.
...
### `untrack(fn, ...args)`
Runs a function outside the current reactive scope. Arguments are passed as-is to the fn. Signal reads do not create dependencies, and effects or disposal callbacks created inside the function are detached from the caller's lifecycle ownership.
...
Use a separately managed scope when detached work needs an explicit lifetime.

### `freeze(fn)`
Locks in the current effect scope for later use. The returned wrapper keeps referencing the original scope—much like an `AsyncContext` scope—so any effects you create inside it attach to the cleanup bucket that existed at wrap time. If an effect was running when you called `freeze`, subsequent executions will tie freshly accessed signals back to that same effect. Once the owning scope is disposed, the wrapper still executes under that inactive scope; new effects become inert and stop tracking changes within it.
...
Returns whether the supplied `EffectScope`, or the currently executing scope when omitted, is active and valid. Because `freeze()` captures the current scope directly, destroying that scope invalidates later calls without a separate context-validity flag.
...
If one or more queued effects throw, the failing effects are destroyed, while the scheduler continues draining unrelated runnable work before rejecting. One failure is rethrown directly; multiple failures are reported as an `AggregateError`.
