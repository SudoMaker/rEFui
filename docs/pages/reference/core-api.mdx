...
Returns whether the supplied `EffectScope`, or the currently executing scope when omitted, is active and valid. Root execution without a scope is valid. Frozen and captured callbacks therefore become invalid automatically when their captured scope is destroyed.
