...
Creates a context "Provider" component. Each `createContext` call creates a unique context identity (even if names match).

The returned Provider sets a scoped value for its children. If no Provider is found above the current component, `useContext` returns the `defaultValue`.

**Parameters:**
- `defaultValue`: Value returned by `useContext` when no Provider is present.
- `name`: Optional display name used for debugging/dev output.

**Returns:** A Provider component you can render as `<MyContext value={...}>...</MyContext>`.

**Behavior notes:**
- When *not* using the Reflow renderer (e.g., custom renderer with classic JSX), wrap Provider children in a function so they render under the Provider scope. With Reflow or the default automatic JSX runtime, you can pass children directly.

```jsx
import { createContext, signal } from 'refui';

// Tip: if consumers should react to changes, provide a signal as the value.
export const ThemeContext = createContext(signal('light'), 'ThemeContext');

// Non-Reflow/custom renderer: wrap children to inherit context
(
	<ThemeContext value={signal('dark')}>
		{() => <App />}
	</ThemeContext>
);
```
...
Reads the current value for a context Provider created by `createContext`.

Because rEFui is retained-mode (component bodies run once), `useContext` is typically used to retrieve a stable object/signal. If you want updates, pass a `signal(...)` as the Provider value and read that signal reactively in the consumer.

**Parameters:**
- `Context`: The Provider component returned by `createContext`.

**Returns:** The provided value for the current scope (or the context `defaultValue`).

```jsx
import { useContext, $ } from 'refui';
import { ThemeContext } from './theme-context.js';

export const Button = (props, ...children) => {
	const theme = useContext(ThemeContext); // a signal in this example
	return (
		<button class:dark={$(() => theme.value === 'dark')}>
			{children}
		</button>
	);
};
...
If the component gets disposed (usually unmounted) before the timeout resolves, `getSelf()` returns `undefined`, preventing the deferred callback from mutating state that no longer has a live context.

Think of `capture` (and the lower-level `freeze`) as rEFui's equivalent of `AsyncContext`: they carry the current component context across promises, timers, or other deferred callbacks—until the owning instance disposes, at which point the captured function reports no current component so you can bail out safely.
...
Boolean flag exported from `refui/signal` for the context currently executing. A frozen or captured callback runs with `contextValid === false` after its owner has disposed; the previous ambient value is restored when the callback returns. Async helpers such as `<Async>` use this guard to avoid rendering after their parent component has disposed.
