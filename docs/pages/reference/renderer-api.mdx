...
const customRenderer = createRenderer({
	...
	getParent: (node) => /* optional physical parent lookup */,
	removeNode: (node) => /* remove node */,
	clearChildren: (parent, first, last) => /* optional guarded bulk clear */,
...
The `'<>'` sentinel used to represent fragments in JSX. It is also available as `R.f` in the classic JSX transform.
...
### `renderer.clearFragment(fragment)`

Attempts to remove every rendered child of a connected fragment in one host
operation. It returns `true` only when the host renderer implements
`nodeOps.clearChildren` and confirms that the fragment owns the complete child
range of its parent. Fragment anchors are restored automatically.

When the method returns `false`, callers must use ordinary node removal. The
built-in `For` component does this automatically, so sibling content is never
bulk-cleared.

```js
if (!renderer.clearFragment(fragment)) {
	// Fall back to removing the fragment's children individually.
}
```
