...
> **Note:** If the parent component unmounts before the awaited promise settles, its captured effect scope becomes inactive. The async boundary then skips rendering the resolved value, preventing work from running against a disposed instance.
