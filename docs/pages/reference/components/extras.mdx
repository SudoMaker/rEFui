...
Similar to [`For`](basic.mdx#for), but optimized for lists where individual items contain reactive signals. `UnKeyed` efficiently updates only the parts of the DOM that change, making it ideal for dynamic lists.
...
- `dispose()`: Permanently disposes active and pooled component scopes and releases their resources.

`clear()` and `del()` move component slots into the reuse pool; they intentionally keep those scopes alive. Call `dispose()` when the cache itself is no longer needed. Updating or reusing a slot replaces its rendered item subtree with the new props while retaining the cache slot and list position. Disposal is permanent and idempotent; later mutation calls are ignored.

Each retained cache slot remains a component. When HMR is inactive and an item's props do not contain a truthy `$ref`, its template is invoked directly inside that slot's replacement scope, avoiding a redundant nested component wrapper while preserving signals, effects, and cleanup. Supplying a truthy `$ref` uses full component construction and assigns the nested template instance to the ref. HMR builds also retain full component construction so the template remains replaceable.
...
const codeParser = ({ source }) => {
	return (R) => {
		...
		// Split into text and keyword tokens so source remains escaped by the renderer.
		const keywordPattern = /\b(function|const|let|var|if|else|return)\b/g
		const parts = code.split(keywordPattern)
		return R.c(
			'pre',
			null,
			...parts.map((part, index) =>
				index % 2 ? R.c('span', { class: 'keyword' }, part) : part
			)
		)
	}
}
...
// HTML-like parser that converts custom tokens to renderer nodes
const customHtmlParser = ({ source }) => {
	return (R) => {
		const text = String(read(source) ?? '')
		const tokenPattern = /\[button:([^\]]+)\]|\[link:([^|\]]+)\|([^\]]+)\]/g
		const nodes = []
		let cursor = 0
		...
		for (const match of text.matchAll(tokenPattern)) {
			...
			if (match[1]) {
				const label = match[1]
				nodes.push(R.c('button', {
					'on:click': () => alert(`${label} clicked!`)
				}, label))
			} else {
				const href = /^https?:\/\//i.test(match[2]) ? match[2] : '#'
				nodes.push(R.c('a', {
					href,
					target: '_blank',
...
```jsx
import { Parse } from 'refui/extras'
import { signal, read, useEffect } from 'refui'
